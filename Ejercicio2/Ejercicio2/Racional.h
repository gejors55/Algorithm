...
public:
	...
	Racional();
	~Racional();
	Racional(const Racional& r);//contructor por copia
...
template <class T>
Racional<T>::Racional(const Racional<T> &r){
	_numer = new T;
	_denom = new T;
	*_numer = *r._numer;
	*_denom = *r._denom;
...
template <class T>
Racional<T> Racional<T>::operator=(const Racional<T> &p){
	*_numer = *p._numer;
	*_denom = *p._denom;
	return *this;
}
...
Racional<T> Racional<T>::suma(const Racional<T> &r) const {
	if (*_denom == *r._denom) {
		...
		return Racional<T>(*_numer*(ndenom / *_denom)+*r._numer*(ndenom / *r._denom),ndenom);
	}
}
